 ...
 */
{
	 ...
	 */
	{
		template : {
			...
			'<th colspan="5" class="switchPlus"></th>', //
 ...
 */
{
	...
	*取得绿色日期
	*/
	getHolidayDate : function(){
		console.log("进入方法A");
		var me = this;
		var arrA=me.config.ar1;
		var arrB=me.config.ar2;
		console.log("数组筛选前："+me.selectedDateAry.length);
		//选中日期数组排除掉arrA和arrB
		if(arrA){
			for(var i=0;i<arrA.length;i++){
				//判断arrA[i]是否在selectedDateAry里面
				for(var j=0;j<me.selectedDateAry.length;j++){
					if(arrA[i]==me.selectedDateAry[j]){
						//删除
						me.selectedDateAry.splice(j, 1);
					}
				}
			}
		}
		if(arrB){
			for(var i=0;i<arrB.length;i++){
				//判断arrB[i]是否在selectedDateAry里面
				for(var j=0;j<me.selectedDateAry.length;j++){
					if(arrB[i]==me.selectedDateAry[j]){
						//删除
						me.selectedDateAry.splice(j, 1);
					}
				}
		...
		//为了解决bug，先筛选两次，在处理之后再复原数据
		if (arrB) {
			me.selectedDateAry = me.selectedDateAry.concat(arrB);
		};
		console.log("数组筛选后："+me.selectedDateAry);
		return me.selectedDateAry;
	},
	/**
	*取得红色日期
	*/
	getWorkDate : function(){
		...
		//为了解决bug，先筛选两次，在处理之后再复原数据
		if (arrA) {
			me.selectedDateAry = me.selectedDateAry.concat(arrA);
		};
		console.log("数组筛选后："+me.selectedDateAry);
		return me.selectedDateAry;
	},
	...
	fill : function() {
		...
		var d = new Date(me.viewDate), year = d.getFullYear(), month = d.getMonth(), //month = 0,
		...
		for(var i = 0; i < me.config.nMonths; i++) {
			...
			while(prevMonth.valueOf() < nextMonth.valueOf()) {
				...
				$kit.each(me.selectedDateAry, function(o) {
					if(arrA){
						for(var i=0;i<arrA.length;i++){
							if(arrA[i].valueOf() == prevMonth.valueOf()) {
								//判断arrA[i]在me.selectedDateAry中
								for(var m=0;m<me.selectedDateAry.length;m++){
									if(me.selectedDateAry[m].valueOf() == arrA[i].valueOf()) {
										clsName += ' activePlus';
										return false;
									}
								}
					...
					if(arrB){
						for(var i=0;i<arrB.length;i++){
							if(arrB[i].valueOf() == prevMonth.valueOf()) {
								//判断arrB[i]在me.selectedDateAry中
								for(var n=0;n<me.selectedDateAry.length;n++){
									if(me.selectedDateAry[n].valueOf() == arrB[i].valueOf()) {
										clsName += ' activePls';
										return false;
									}
								}
					...
					if(o && o.valueOf() == prevMonth.valueOf()) {
						clsName += ' active';
						return false;
					}
				});
			...
			switch(target.nodeName.toLowerCase()) {
				...
				case 'td':
					if($kit.hsCls(target, 'day') && !$kit.hsCls(target, 'disabled')) {
						...
						//判断选择的日期是否在数组中。若存在，则取消红色和绿色样式并移除；若不存在，则正常操作即可
						// if(newDate){
						// 	for(var i=0;i<me.selectedDateAry.length;i++){
						// 		if(me.selectedDateAry[i].valueOf() == newDate.valueOf()) {
						// 			//找出这个td元素
						// 			$kit.each($kit.$el('.datepicker-days td.day', me.picker), function(o) {
						// 				$kit.rmCls(o, 'activePlus');
						// 				$kit.rmCls(o, 'activePls');
						// 			});
						// 			$kit.rmCls(target, 'activePlus');
						// 			$kit.rmCls(target, 'activePls');
						// 		}
						// 	}
						// }
						...
						if(me.date && me.date.valueOf() == newDate.valueOf()) {
							...
							me.fill();
							//console.log(me.selectedDateAry);
	...
	update : function() {
		...
		if(me.date < me.startDate) {
			...
			if(me.viewDate == null) {
				...
				if(me.date && me.viewDate && Math.abs((me.date.getFullYear() - me.viewDate.getFullYear()) * 12 + me.date.getMonth() - me.viewDate.getMonth()) > 11) {
					me.viewDate = new Date(me.date);
				}
			}
		}
	...
	addValue : function(continuous) {
		...
		if(continuous == true) {
			...
			//循环，如果选中的在数组A或者B里面，则删除
			//将A和B加和
			var arrA=me.config.ar1;
			var arrB=me.config.ar2;
			if (arrA) {
				for(var m = 0; m < arrA.length; m++) {
					if(me.date.valueOf() == arrA[m].valueOf()) {
						arrA.splice(m, 1);
						me.config.ar1 = arrA;
					}
				}
			...
			if (arrB) {
				for(var n = 0; n < arrB.length; n++) {
					if(me.date.valueOf() == arrB[n].valueOf()) {
						 arrB.splice(n, 1);
						 me.config.ar2= arrB;
					}
				}
